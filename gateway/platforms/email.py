...
class EmailAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            # Test SMTP connection — use SMTPS (port 465) or STARTTLS (port 587)
            if self._smtp_port == 465:
                smtp = smtplib.SMTP_SSL(self._smtp_host, self._smtp_port, context=ssl.create_default_context(), timeout=30)
            else:
