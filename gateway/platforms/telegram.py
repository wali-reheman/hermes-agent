...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        # Normalize chat_id: prefer int, fall back to Bot API resolution for phone/username.
        # Telegram Bot API rejects phone numbers like "+1 202 391 3091" as int cast targets.
        try:
            chat_id_int = int(chat_id)
        except ValueError:
            # phone number or username — resolve via getChat
            try:
                # Strip spaces from phone numbers
                resolved = chat_id.strip().replace(" ", "")
                chat = await self._bot.get_chat(resolved)
                chat_id_int = chat.id
            except Exception:
                return SendResult(success=False, error=f"Could not resolve chat_id: {chat_id}")
