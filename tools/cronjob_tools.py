...
_CRON_EXFIL_COMMAND_PATTERNS = [
    ...
    (rf'curl\s+[^\n]*(?:-H|--header)\s+["\']Authorization:\s*(?:Bearer|token)\s+{_CRON_SECRET_VAR_RE}["\'](?:\s*https?://api\.github\.com\b|(?:\s(?!https?://))*)', "exfil_curl_auth_header"),
]
#                                                               ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#                                                               Negative lookahead: block auth headers sent to non-github hosts.
#                                                               The (?:\s(?!https?://))* group:
#                                                                 - consumes whitespace on the same line
#                                                                 - does NOT consume newlines (stays on same line)
#                                                                 - backslash-newline would end the pattern at the backslash
#                                                               api.github.com is exempt so the bundled GitHub skills
#                                                               (which use curl -H "Authorization: token $GITHUB_TOKEN" https://api.github.com/...)
#                                                               do not trigger this pattern.
...
def _scan_cron_prompt(prompt: str) -> str:
    ...
    # NOTE: The exfil_curl_auth_header threat pattern now carries a negative
    # lookahead (?!\s*https?://api\.github\.com\b) that exempts api.github.com
    # destinations without needing any allowlist substitution here.
    ...
    for pattern, pid in _CRON_EXFIL_COMMAND_PATTERNS:
        m = re.search(pattern, prompt_to_scan, re.IGNORECASE)
        if m:
            # Special case: exfil_curl_auth_header targeting api.github.com is exempt.
            if pid == "exfil_curl_auth_header":
                after_match = prompt_to_scan[m.end():]
                # Check if the URL is on the same line or via shell line continuation (backslash+newline).
                # For same-line: the URL immediately follows the matched text (possibly with space).
                # For multi-line: the matched text ends with space or backslash, and the continuation
                #                contains the github.com URL.
                if re.search(r"https?://api\.github\.com\b", after_match, re.IGNORECASE):
                    # Verify this is a legitimate continuation (matched text ends with space or backslash,
                    # and the URL is reached via backslash-newline or on the same line).
                    matched_text = m.group(0)
                    if (matched_text.rstrip().endswith(('"', "'")) and
                        re.match(r"\s*https?://api\.github\.com\b", after_match, re.IGNORECASE)):
                        continue  # same-line github URL
                    # Multi-line continuation: check for backslash-newline before the URL
                    if re.search(r"\\\n\s*https?://api\.github\.com\b", after_match, re.IGNORECASE):
                        continue  # continuation-line github URL
            return f"Blocked: prompt matches threat pattern '{pid}'. Cron prompts must not contain injection or exfiltration payloads."
    return ""
